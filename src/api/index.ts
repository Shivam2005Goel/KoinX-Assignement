...
export interface CapitalGainsBase {
  stcg: {
    profits: number;
    losses: number;
  };
...
export interface CapitalGainsResponse {
  capitalGains: CapitalGainsBase;
}

export const mockHoldings: CoinHolding[] = [
  {
    "coin": "USDC",
    "coinName": "USDC",
...
// API Services
export const fetchHoldings = async (): Promise<CoinHolding[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      // Sort assets logically: by Coin Name alphabetically
      const sortedHoldings = [...mockHoldings].sort((a, b) => 
        a.coinName.localeCompare(b.coinName)
      );
      resolve(sortedHoldings);
    }, 800);
  });
};
